...
const Library = ({ lang }) => {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="pt-40 lg:pt-56 px-[6%] pb-40 max-w-[1600px] mx-auto min-h-screen relative"
    >
      ...
      <div className="mb-32 text-center space-y-6">
        <motion.div 
          ...
          className="inline-flex items-center gap-3 px-5 py-2 rounded-full bg-[var(--primary)]/5 border border-[var(--primary)]/20 mb-4 backdrop-blur-xl"
        >
          <Binary className="size-4 text-[var(--primary)] animate-pulse" />
          <span className="font-mono text-[0.65rem] font-bold uppercase tracking-[0.5em] text-[var(--primary)]">Intelligence_Archive_v4.2</span>
        </motion.div>
        
        <motion.h1 
          initial={{ y: -20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          className="text-6xl lg:text-9xl font-display font-black text-white italic tracking-tighter uppercase leading-[0.8]"
        >
          {t.libraryTitle.split(' ')[0]} <span className="text-[var(--primary)] text-stroke">{t.libraryTitle.split(' ')[1] || 'ARCHIVE'}</span>
        </motion.h1>
        
        <p className="text-[var(--muted)] text-[11px] font-bold uppercase tracking-[0.5em] max-w-2xl mx-auto leading-relaxed italic opacity-60">
          {t.libraryDesc}
        </p>
      </div>

      {/* HUD Mode Switcher */}
      <div className="flex bg-white/[0.02] border border-white/5 p-2 rounded-[2rem] mb-24 max-w-xl mx-auto backdrop-blur-3xl relative shadow-2xl">
        ...
        <button 
          ...
          className={`relative flex-1 py-5 rounded-[1.5rem] text-[0.65rem] font-black uppercase tracking-[0.3em] transition-all duration-500 z-10 flex items-center justify-center gap-3 ${mode === 'browse' ? 'text-black' : 'text-white/30 hover:text-white/60'}`}
        >
          {mode === 'browse' && (
            <motion.div 
              layoutId="mode-pill" 
              className="absolute inset-0 bg-[var(--primary)] rounded-[1.5rem] -z-10 shadow-[0_0_30px_rgba(0,255,135,0.3)]" 
            />
          )}
          <BookOpen className="size-4" />
          {currentLang === 'en' ? 'Encyclopedia' : 'ಗ್ರಂಥಾಲಯ'}
        </button>
        <button 
          onClick={() => setMode('manual')}
          className={`relative flex-1 py-5 rounded-[1.5rem] text-[0.65rem] font-black uppercase tracking-[0.3em] transition-all duration-500 z-10 flex items-center justify-center gap-3 ${mode === 'manual' ? 'text-black' : 'text-white/30 hover:text-white/60'}`}
        >
          {mode === 'manual' && (
            <motion.div 
              layoutId="mode-pill" 
          ...
          {currentLang === 'en' ? 'Smart Search' : 'ಬುದ್ಧಿವಂತ ಹುಡುಕಾಟ'}
        </button>
      </div>
        ...
        {mode === 'browse' ? (
          ...
          >
            ...
            <div className="relative max-w-4xl mx-auto group">
              ...
              <input 
                type="text"
                placeholder={currentLang === 'en' ? "IDENTIFY_SPECIMEN_OR_SYMPTOMS..." : "ಬೆಳೆ ಅಥವಾ ಲಕ್ಷಣಗಳನ್ನು ಹುಡುಕಿ..."}
                className="w-full bg-white/[0.02] border border-white/5 rounded-[3rem] py-10 pl-28 pr-12 text-white 
                           placeholder:text-white/10 focus:outline-none focus:border-[var(--primary)]/30 
                           transition-all shadow-2xl text-sm font-mono tracking-[0.2em] uppercase italic backdrop-blur-xl"
                value={searchTerm}
              ...
              {filteredDiseases.length > 0 ? (
                filteredDiseases.map((item, index) => (
                  <motion.div 
                    ...
                    className="glass-panel group relative overflow-hidden p-12 hover:border-[var(--primary)]/30 transition-all duration-700"
                  >
                      ...
                      <div className="flex justify-between items-start mb-10">
                        <div className="space-y-4">
                          <div className="flex items-center gap-3">
                            <span className="inline-block px-4 py-1.5 bg-[var(--primary)]/5 text-[var(--primary)] text-[0.6rem] font-black uppercase tracking-[0.4em] rounded-full border border-[var(--primary)]/20 italic">
                              SPECIMEN::{item.crop[currentLang] || item.crop.en}
                            </span>
                            <div className="size-2 rounded-full bg-[var(--primary)] animate-pulse" />
                          </div>
                          <h3 className="text-4xl md:text-5xl font-display font-black text-white italic group-hover:text-[var(--primary)] transition-colors uppercase tracking-tighter leading-none">
                            {item.disease[currentLang] || item.disease.en}
                          </h3>
                        </div>
                        <div className="flex flex-col items-end gap-3">
                          <div className={`px-4 py-1.5 rounded-full border text-[0.6rem] font-black uppercase tracking-widest
                      ...
                      <div className="space-y-10">
                        <div className="space-y-4">
                          <div className="flex items-center gap-3 text-[var(--primary)]/50">
                            <Activity className="size-4" />
                            <h4 className="text-[0.65rem] font-black uppercase tracking-[0.4em]">Clinical_Anomalies</h4>
                          </div>
                          <p className="text-lg text-[var(--muted)] leading-relaxed font-sans line-clamp-3 group-hover:line-clamp-none transition-all duration-500">{item.symptoms[currentLang] || item.symptoms.en}</p>
                        </div>
                        ...
                        <div className="bg-white/[0.03] border border-white/5 p-8 rounded-[2rem] group-hover:bg-[var(--primary)]/[0.03] group-hover:border-[var(--primary)]/20 transition-all duration-500 relative overflow-hidden">
                          ...
                          <div className="flex items-center gap-3 mb-4">
                            ...
                            <h4 className="text-[0.65rem] font-black uppercase tracking-[0.4em] text-[var(--primary)]">Protocol_Alpha_Verified</h4>
                          </div>
                          <p className="text-lg text-white/90 font-medium leading-relaxed italic">{item.treatment[currentLang] || item.treatment.en}</p>
                        </div>
                      </div>
                    </div>
                  </motion.div>
